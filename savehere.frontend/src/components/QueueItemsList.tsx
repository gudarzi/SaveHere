...
import {formatRequestHeaders} from "../utils/formatRequestHeader";
...
const QueueItemsList = (props: {
  dummy: string;
  onDownloadFinished: () => unknown;
  requestHeaders: [];
}) => {
    ...
    socketRef.current.addEventListener("message", (event) => {
      ...
      if (event.data.startsWith("progress:")) {
        ...
        setData((prevData) =>
          prevData.map((item) =>
            item.id === Number(id)
              ? {...item, status: 1, progressPercentage: Number(progress)}
              : item
          )
        );
        ...
        setData((prevData) =>
          prevData.map((item) =>
            item.id === Number(id)
              ? {...item, downloadSpeedInBytesPerSecond: Number(speed)}
              : item
          )
        );
        ...
        setData((prevData) =>
          prevData.map((item) =>
            item.id === Number(id)
              ? {...item, downloadSpeedInBytesPerSecondAvg: Number(speed)}
              : item
          )
        );
      }
    ...
    try {
      const response = await fetch(`api/FileDownloadQueueItems/startdownload`, {
        ...
        headers: {
          ...
          ...formatRequestHeaders(props.requestHeaders),
        },
        body: JSON.stringify({
          id,
  ...
  const cancelDownload = async (id: number) => {
    try {
      const response = await fetch(
        `api/FileDownloadQueueItems/canceldownload`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const renderNode = (node: QueueItem) => {
    ...
    return (
      <>
        <li
          key={node.id}
          className="container mx-auto flex justify-between w-full p-1 text-sm font-medium text-left text-gray-800 rounded-lg focus:outline-none dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700"
        >
          <div className="container mx-auto flex justify-left items-center">
            <svg
              ...
              viewBox="0 0 24 24"
              fill="currentColor"
              className="w-5 h-5 m-1 dark:text-slate-300"
            >
          ...
          <div className="ml-1 p-1 rounded-xl bg-gray-400 dark:bg-gray-700">
            {statusMapping[node.status]}
          </div>
          <div className="ml-1 p-1 rounded-xl bg-gray-400 dark:bg-gray-700">
            {node.progressPercentage}%
          </div>
          <div className="ml-1 p-1 rounded-xl whitespace-nowrap bg-gray-400 dark:bg-gray-700">
            {convertSize(node.downloadSpeedInBytesPerSecond) ?? "KB/s"}
          </div>
          <div className="ml-1 p-1 rounded-xl whitespace-nowrap bg-gray-400 dark:bg-gray-700">
            {convertSize(node.downloadSpeedInBytesPerSecondAvg) ?? "KB/s"}
          </div>
          <select
            onChange={(e) => setUseHeadersForFilename(Number(e.target.value))}
            className="ml-1 p-1 rounded-xl bg-gray-400 dark:bg-gray-700 text-gray-800 dark:text-white hover:bg-[#FFFFFF33]"
          >
            <option value="1" className="dark:bg-gray-500">
              Use Headers For Filename
            </option>
            <option value="0" className="dark:bg-gray-500">
          ...
          <div className="ml-1 px-2 rounded-xl whitespace-nowrap bg-gray-400 dark:bg-gray-700">
            ...
            <button
              onClick={() => cancelDownload(node.id)}
              className="p-1 rounded-full hover:bg-[#FFFFFF33]"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
            ...
            <button
              onClick={() => requestDownload(node.id)}
              className="p-1 rounded-full hover:bg-[#FFFFFF33]"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
      ...
      <div className="flex flex-row">
        ...
        <button className="ml-2" onClick={fetchList}>
          <svg
            ...
            viewBox="0 0 24 24"
            fill="currentColor"
            className="w-6 h-6 p-1 rounded-full hover:bg-[#FFFFFF33] dark:text-slate-100"
          >
            <path
              fillRule="evenodd"
